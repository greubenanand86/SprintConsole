#!/usr/bin/env bash
# Incident Agent — Incident Management Playbook v1.0 | Jira Workflow Governance §15 | Release Management Playbook §9
# Production incidents require: severity classification (SEV-1/2/3/4), rollback assessment,
# root cause tracking, postmortem documentation, post-release validation
# Per Incident Management Playbook §3: Incident Detected → Classify → Contain → Rollback Assessment → Resolve → Monitor → Postmortem → Product Memory
# Per Playbook §6: Postmortem mandatory (root cause, timeline, impact, detection gap, resolution, prevention)
# Hotfix governance (Release Playbook §9): incident classification + rollback awareness +
#   post-release validation + postmortem documentation
# Incident learnings stored in Product Memory per §7
...
Per Incident Management Playbook §3-§6 and Release Management Playbook §9, production incidents require ALL of:
1. Severity classification (SEV-1=production outage/major data risk, SEV-2=major degradation, SEV-3=partial, SEV-4=minor)
2. Rollback awareness — is a hotfix or revert viable? Who owns the rollback? (Playbook §5)
...
5. Postmortem documentation — learning recorded in Product Memory (Playbook §6: root cause, timeline, impact, detection gap, resolution, prevention)

Incident Management Playbook §4 ownership: Incident Response Agent (coordination), TPM Agent (escalation), DevOps Agent (rollback),
  QA Agent (validation), Security Agent (assessment), Human (final decisions).

Release Management Playbook §9 hotfix governance (applies if incident requires a hotfix/* branch):
...
- Postmortem must be completed and recorded (Playbook §9, Incident Playbook §6)

Output format — output EXACTLY these sections (map severity to SEV-* per Incident Playbook §2):

SEVERITY: <SEV-1|SEV-2|SEV-3|SEV-4 — mapped from Jira priority: P0→SEV-1, P1→SEV-2, P2→SEV-3, P3→SEV-4>
SEVERITY_RATIONALE: <why this severity level per Incident Playbook §2>
...
$STANDARD_OUTPUT_SUFFIX" \
  ...
  # Severity icon (per Incident Management Playbook §2)
  case "$SEVERITY" in
    SEV-1) ICON="🔴" ;; # Production outage / major data risk
    SEV-2) ICON="🟠" ;; # Major feature degradation
    SEV-3) ICON="🟡" ;; # Partial degradation
    SEV-4) ICON="🟢" ;; # Minor issue
    *)     ICON="⚪" ;;
  esac
...
$([ "$SEVERITY" = "SEV-1" ] || [ "$SEVERITY" = "SEV-2" ] && echo "⚠ HIGH SEVERITY (Incident Playbook §2) — Human escalation required per Jira Workflow §15. Do NOT close without postmortem sign-off.")
...
$(standard_fields_block)"
  ...
  # Escalate SEV-1/SEV-2 and hotfix-required incidents to TPM per Incident Management Playbook §4
  if [ "$SEVERITY" = "SEV-1" ] || [ "$SEVERITY" = "SEV-2" ]; then
    escalate_to_tpm "$KEY" \
      "$SEVERITY production incident: $ROOT_CAUSE. Incident Playbook §4 + §6 + Release Playbook §9: human escalation required. Postmortem mandatory (root cause, timeline, impact, detection gap, resolution, prevention)." \
      "INCIDENT AGENT"
  fi
